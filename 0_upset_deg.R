...
data=UpSetR::fromList(deg.genes)
sets=deg.sets
order.by = "freq"
sets.bar.color = c("#3c9ab2","#fd6467","#e1af00")
...
for(i in 1:3) {
  ...
  if(length(j) > 0) Matrix_layout$color[j] <- c("#3c9ab2","#fd6467","#e1af00")[i]
}
...
deg.upset.plot <- structure(class = "upset",
                    .Data=list(
                      Main_bar = Main_bar,
                      Matrix = Matrix,
                      Sizes = Sizes,
