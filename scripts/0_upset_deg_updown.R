...
order.by = "freq"
sets.bar.color = c("#3c9ab2","#3c9ab2","#fd6467","#fd6467","#e1af00","#e1af00")
...
for(i in 1:6) {
  ...
  if(length(j) > 0) Matrix_layout$color[j] <- c("#3c9ab2","#3c9ab2","#fd6467","#fd6467","#e1af00","#e1af00")[i]
}
...
Make_matrix_plot <- function(Mat_data,Set_size_data, Main_bar_data, point_size, line_size, text_scale, labels,
  ...
  Mat_data[Mat_data$alpha!=1,5]<-0.2
  ...
  Matrix_plot <- (ggplot()
                  ...
                  + geom_point(data= Mat_data, aes_string(x= "x", y= "y"), colour = Mat_data$color, fill=Mat_data$color,
                               size= point_size, alpha = Mat_data$alpha, shape=Mat_data$shape)
...
deg.updown.upset.plot <- structure(class = "upset",
                                .Data=list(
                                  Main_bar = Main_bar,
                                  Matrix = Matrix,
                                  Sizes = Sizes,
